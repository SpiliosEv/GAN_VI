...
P_S = 10**((55-30)/10) #10**((35-30)/10) #source power % dbm to power  
P_R = 10**((55-30)/10) #10**((45-30)/10) #total relay budget power 
sigmaSQ = 1
sigma_xiSQ = 0.5  # multipath variance
sigma_DSQ = 1
etaSQ = 30 #(10**(7.7/10))**2 #60 # Shadowing Power    
c1 = 10 # Correlation Distance
c2 = 10#1 #120   # Correlation Time
...
env = GridWorld(rMap_RowCells, rMap_ColCells) # create the environment
...
T, C_SD, C_SD_chol, kappa, pathlossF, pathlossG, grid_X, grid_Y, S_cord, D_cord = env.setState()


f_maps, g_maps = Perfect_CSI(pathlossF, pathlossG, C_SD, C_SD_chol, kappa)

VI = np.zeros(shape = (50000,par,par))

for i in range(0,50000):
  for k in range(par):
    for l in range(par):
      VI[i][k][l] = VI_local(f_maps[k,l,i],g_maps[k,l,i], P_R, P_S)

VI = np.array(VI, dtype=np.float32)
print("end")
np.save("VI_20_full.npy", VI)
